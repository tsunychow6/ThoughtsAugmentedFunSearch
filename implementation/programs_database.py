...
class Prompt:
    ...
    # Store the additional information of the prompt.
    rationale: str = ""
    strategy: str = ""
...
class ProgramsDatabase:
    ...
    def print_all_prompts(self) -> None:
        """Prints the prompt that each island would currently generate."""
        for island_id, island in enumerate(self._islands):
            if not island._clusters:
                print(f'=== Island {island_id} (no programs yet) ===')
                continue
            code, version_generated = island.get_prompt()
            print(f'=== Island {island_id} | version_generated={version_generated} ===')
            print(code)
            print()
...
class Island:
    ...
    def _generate_prompt(
        ...
        for i, implementation in enumerate(implementations):
            ...
            doc_parts = []
            if i >= 1:
                doc_parts.append(f'Improved version of `{self._function_to_evolve}_v{i - 1}`.')
            # Updated to also store the rationale and strategy of the implementation.
            if implementation.rationale:
                doc_parts.append(f'Rationale: {implementation.rationale}')
            if implementation.strategy:
                doc_parts.append(f'Strategy: {implementation.strategy}')
