...
class Profiler:
    ...
    def _write_json(self, programs: code_manipulation.Function):
        ...
        if sample_order != 0:
          result_score.scores.append(score)
          json_str = json.dumps(result_score.to_dict(), indent=4)
          scores_path = os.path.join("/content/drive/MyDrive/logs", 'enhanced_funsearch_scores.json')
          with open(scores_path, "w") as f:
            json.dump(result_score.to_dict(), f, indent=4)
        content = {
    ...
    def register_function(self, programs: code_manipulation.Function):
        ...
        need_record = sample_orders is None or sample_orders not in self._all_sampled_functions
        if need_record:
            self._num_samples += 1
    ...
    def _record_and_verbose(self, sample_orders: int):
        ...
        print(f'Rationale: {str(function.rationale)}')
        print(f'Strategy: {str(function.strategy)}')
        print(f'Strategy Faithfulness: {str(function.strategy_faithfulness_score)}')
        print(f'Verdict: {str(function.verdict)}')
        print(f'======================================================\n\n')
