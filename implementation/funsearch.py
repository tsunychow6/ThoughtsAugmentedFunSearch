...
def main(
    ...
    """
    function_to_evolve, function_to_run = _extract_function_names(specification + worst_fit)
    template = code_manipulation.text_to_program(specification + worst_fit)
    ...
    worst_fit = template.get_function(function_to_evolve).body

    # Add first fit and best fit
    evaluators[0].analyse(first_fit, island_id=None, version_generated=None, profiler=profiler, rationale=kwargs.get('first_fit_rationale', None), strategy=kwargs.get('first_fit_strategy', None))
    evaluators[0].analyse(best_fit, island_id=None, version_generated=None, profiler=profiler, rationale=kwargs.get('best_fit_rationale', None), strategy=kwargs.get('best_fit_strategy', None))
    evaluators[0].analyse(worst_fit, island_id=None, version_generated=None, profiler=profiler, rationale=kwargs.get('wrost_fit_rationale', None), strategy=kwargs.get('wrost_fit_strategy', None))
